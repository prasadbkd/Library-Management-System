...
import UserNav from './usernav'
 ...
 class memberHome extends Component {
    ...
    render() {
       ...
       let cssClass= this.state.display?'displaySearch':'hideSearch';
        for(let i=0;i<books.length;i++){
            ...
            {
            if (books[i].title.toLowerCase().includes(lowercasedFilter)) {
            if(books[i].no_copies>0 && books[i].issued_to.includes(user.name)){
              rowdata.push(<tr key={books[i].id}>
                            <td>{books[i].title}</td>
                ...
                rowdata.push(<tr key={books[i].id}>
                              ...
                              <td>{books[i].rack}</td>
                              <td>{books[i].no_copies}</td>
                              <td><button className='btn btn-primary'onClick={()=>this.click(books[i].id,books[i],user.name)}>Issue</button></td>
                          </tr>)
              }
            else{
                rowdata.push( <tr key={books[i].id}>
    ...
    if (books[i].genere.toLowerCase().includes(lowercasedFilter)) {
        ...
        <h6 onClick={this.displaySearch} className='search_heading'><mark>{y}Difficulty in finding..?? Click here to use our search!{x}</mark></h6>
               <div className={cssClass}>
                ...
                <input value={filter} className="form-control" id='search_input' onChange={this.handleChange} placeholder="Enter your Query here..." />
                </div>
